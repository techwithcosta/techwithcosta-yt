...
- I mostly use ZSH, which has a lot of customization frameworks with themes, colors, extra functionalities, such as:
  - `OhMyZsh`
  - `OhMyPosh`
  - `Powerlevel10k`
  - `Starship`
...
- Install `Code Spell Checker` by Street Side Software, for spelling suggestions
- Write something wrong, then `right click` -> `Spelling Suggestions...`
- Install `Todo Tree` by Gruntfuggly
- Write "TODO" as a coding language comment to group all your TODOs in the `TODOs` panel
...
--fingerprint
```
- The gpg command will report the key fingerprint. Confirm fingerprint here https://www.hashicorp.com/trust/security?product_intent=terraform
...
code main.tf
```
- Paste the following Terraform configuration into it, which is just an example to interact with Google Cloud Platform (GCP), to create a storage bucket and BigQuery dataset
...
- In a real scenario, you would setup your GCP service account credentials (in this example, here "./keys/my-credentials.json")
- From this point on you could run the following
```bash
terraform plan
```
```bash
